...
class ScriptInjectionSink extends Sink {
  MethodCall methodAccess;

  ScriptInjectionSink() {
    isScriptArgument(methodAccess, this.asExpr()) or
    evaluatesRhinoExpression(methodAccess, this.asExpr()) or
    compilesScript(methodAccess, this.asExpr()) or
    definesRhinoClass(methodAccess, this.asExpr())
  }
